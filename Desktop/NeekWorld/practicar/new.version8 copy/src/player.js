class Player {
        ...
        this.ghostPetz.forEach((pet, index) => {
            $('#pet-buttons').append(`
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <button class="btn btn-secondary mx-2" onclick="player.switchPet(${index})"><i class="fas fa-paw"></i> ${pet.type} ${index + 1}</button>
                    <button class="btn btn-danger mx-2" onclick="player.releasePet(${index})"><i class="fas fa-times"></i> Liberar</button>
                    <button class="btn btn-success mx-2" onclick="player.sellPet(${index})"><i class="fas fa-dollar-sign"></i> Vender</button>
                </div>
            `);
        });
    }
    ...
    sellPet(index) {
        const pet = this.ghostPetz[index];
        const value = pet.level * 10 + pet.health + pet.energy + pet.happiness;
        this.coins += value;
        this.ghostPetz.splice(index, 1);
        this.updatePetButtons();
        this.updateCoins();
        this.showMsg(`Has vendido ${pet.type} por ${value} monedas.`, 'info');
        this.saveProgress();
    }
    ...
    releasePet(index) {
        ...
        this.ghostPetz.splice(index, 1);
        this.updatePetButtons();
        this.showMsg(`Has liberado a ${pet.type}.`, 'info');
        this.saveProgress();
    }
