...
export default {
  ...
  async created() {
    try {
      const response = await axios.get("https://us-central1-casiangelesydemonios.cloudfunctions.net/api/users/me", {
        headers: {
  ...
  methods: {
    async handleUpdateProfile() {
      ...
      try {
        ...
        await axios.put("https://us-central1-casiangelesydemonios.cloudfunctions.net/api/users/me", updatedData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
