...
export default {
  ...
  methods: {
    ...
    async loadCourses() {
      ...
      this.errorMessage = "";
      try {
        const response = await axios.get("https://us-central1-casiangelesydemonios.cloudfunctions.net/api/courses/available", {
    ...
    async enrollInCourse(courseId) {
      try {
        this.isLoading = true;
        await axios.post(
          `https://us-central1-casiangelesydemonios.cloudfunctions.net/api/courses/enroll/${courseId}`,
          {},
          {
            headers: { Authorization: `Bearer ${getAuthToken()}` },
          }
    ...
    async deleteCourse(courseId) {
      if (confirm("¿Estás seguro de que deseas eliminar este curso?")) {
        try {
          await axios.delete(`https://us-central1-casiangelesydemonios.cloudfunctions.net/api/courses/${courseId}`, {
            headers: { Authorization: `Bearer ${getAuthToken()}` },
          });
